...
from core.email_automator import (
    EmailDispatcher,
    get_sent_emails
)
...
# Read-only directories containing files
# already included in the GitHub repository.
...
# Create project directories only when running locally
if not IS_VERCEL:
...
# Keep the original app name available
# for local use and compatibility.
app = application
...
app.config["MAX_CONTENT_LENGTH"] = (
    64 * 1024 * 1024
)  # 64 MB maximum upload


# In-memory active dataset cache
_ACTIVE_DATASETS = {}
...
def api_clean_data():
    ...
    try:
        ...
        res = clean_dataset(
            ...
            standardize_names=data.get(
                "standardize_names",
                True
            ),

            default_fill_strategy=data.get(
                "fill_strategy",
                "auto"
            )
        )

        _ACTIVE_DATASETS["current"] = (
            res["df"]
        )
...
def api_export_excel():
    ...
    try:
        ...
        return jsonify({
            ...
            "filename": filename,
            "download_url":
                f"/api/download/exports/{filename}"
        })
...
def api_generate_pdf_report():
    ...
    title = data.get(
        "title",
        "Executive Automated Summary"
    )

    subtitle = data.get(
        "subtitle",
        "DataFlow Automator Pro Intelligence Digest"
    )

    summary = data.get(
        "summary",
        "Automated system digest covering active operational metrics."
    )
...
def api_generate_invoice():
    ...
    items = data.get(
        ...
        [
            ...
            {
                "description":
                    "Custom Scraping Pipeline Integration",
                "qty": 2,
                "rate": 450.0
            }
        ]
...
def api_scrape():
    ...
    try:
        ...
        if source == "books":
            ...
            df = scraper.scrape_books_catalog(
                max_pages=pages
            )
...
def api_run_pipeline():
    ...
    try:
        ...
        if pipe_name == "ecommerce":
            ...
            res = (
                orch.run_ecommerce_intelligence_pipeline(
...
if __name__ == "__main__":
    ...
    logger.info(
        "Starting DataFlow Automator Pro Flask server "
